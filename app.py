...
# Explainability Tab
with tabs[4]:
    st.subheader("Feature Importance & Alternative Explainability")

    chosen_explain_model = st.selectbox("Select Model for Explanation", ["Random Forest", "XGBoost", "Logistic Regression", "SVM"])
    ...
    if chosen_explain_model == "XGBoost":
        ...
        st.pyplot(fig)
    elif chosen_explain_model in ["Random Forest"]:
        ...
        st.write("Model does not have native feature_importances_. Using coefficients if available.")
    ...
    fig, ax = plt.subplots()
    perm_importances.sort_values().plot(kind="barh", ax=ax)
